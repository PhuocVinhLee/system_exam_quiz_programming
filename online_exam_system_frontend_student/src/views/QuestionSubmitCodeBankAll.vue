                            ...
                            <div class="">
                                <router-link :to="{
                                    ...
                                    <h5 class="card-header"><button type="button" class="btn btn-primary">
                                            <i class='bx bxs-plus-circle'></i> Thêm Ngân Hảng Câu Hỏi
                                        </button></h5>
                                </router-link>

                                <QuestionSubmitCodeBankAllTable style="min-height: 450px;"></QuestionSubmitCodeBankAllTable>
                        ...
                        <form class="modal-content">
                                ...
                                <div class="row">
                                    <div class="col mb-3">
                                        <p>Bạn có chắc chắn nuốn xóa <span class="badge bg-label-dark">{{
                                            get_one_question_submit_code_bank.question_submit_code_bank_title }}
                                            </span>? </p>
                                    </div>
                                </div>
                                <div class="row g-2">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-primary"
                                    @click="delete_question_submit_code_bank(get_one_question_submit_code_bank._id, $event)">Save</button>
                            </div>
                        </form>
                    </div>
                </div>
...
import QuestionSubmitCodeBankAllTable from "@/components/QuestionSubmitCodeBankAllTable.vue";
...
const { get_one_question_submit_code_bank, question_submit_code_banks, one_question_submit_code_bank, lenght, state_question_submit_code_bank } = storeToRefs(useQuestion_submit_code_bankStore());
...
async function delete_question_submit_code_bank(id, event) {

    event.preventDefault()
    await action_delete_question_submit_code_bank(id);
    await action_all_question_submit_code_banks(route.query.id, route.query.page);
