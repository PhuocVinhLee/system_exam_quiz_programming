...
class Question_quiz_bankService {
  constructor(client) {
    this.Question_quiz_bank = client.db().collection("question_quiz_banks");
  }
  ...
  async extractConactData(payload) {
    ...
    const question_quiz_bank = {
      question_quiz_bank_title: payload.question_quiz_bank_title,
      subject_id: payload.subject_id,
      teacher_id_of_list: payload.teacher_id_of_list,
      teacher_id: payload.teacher_id,
      question_quiz_bank_type: payload.question_quiz_bank_type,// private public protected
      question_quiz_bank_authorization: payload.question_quiz_bank_authorization,// use edit,
      question_quiz_id: payload.question_quiz_id,
      question_id: payload.question_id,
      list_question: payload.list_question,
      list_teacher: payload.list_teacher
    };
    // Remove undefined fields
    Object.keys(question_quiz_bank).forEach(
      (key) => {
        if (question_quiz_bank[key] === undefined || question_quiz_bank[key] === null) {
          delete question_quiz_bank[key]
        }
      }
    );
    return question_quiz_bank;
  }
...
console.log(question_bank_id)
    ...
    const result = await this.Question_quiz_bank.updateOne(
  ...
  async update(id, payload) {
    ...
    const result = await this.Question_quiz_bank.findOneAndUpdate(
      filter,
      {
  ...
  async create(payload) {
    ...
    const question_quiz_bank = await this.extractConactData(payload);
    const data = await this.Question_quiz_bank.insertOne(question_quiz_bank);
    const length_clt = await this.Question_quiz_bank.countDocuments({});
    const result = {
      data,
      length_clt
    ...
    // const result = await this.Question_quiz_bank.insertOne(
    //   question_quiz_bank,
    // );
    // return result;
  }
  ...
  async find(subject_id, id_teacher) {
    ...
    const data = await this.Question_quiz_bank.find({ $or: [{ $and: [filter1, filter3] }, { $and: [filter1, filter2] }] }).toArray(); //tra ve con tro nen phai toArray()
    return data;
  }
  async find_question_bank_from_teacher_id(subject_id, teacher_id, page) {
    // const cursor = await this.Question_quiz_bank.find(filter);
    const filter1 = {
      subject_id: ObjectId.isValid(subject_id) ? new ObjectId(subject_id) : null,
    };
    const filter2 = {
    ...
    const data = await this.Question_quiz_bank.find({ $and: [filter1, filter2] }).skip(number_skip).limit(litmit).toArray(); //tra ve con tro nen phai toArray()
    const length_clt = await this.Question_quiz_bank.countDocuments(filter1);
  ...
  async findById(id) {
    return await this.Question_quiz_bank.findOne({
      _id: ObjectId.isValid(id) ? new ObjectId(id) : null,
    });
  }

  async findByTeacherId(id) {
    return await this.Question_quiz_bank.find({
  ...
  async delete(id) {
    const result = await this.Question_quiz_bank.findOneAndDelete({
      _id: ObjectId.isValid(id) ? new ObjectId(id) : null,
    });
    return result.value;
  }
  ...
  async deleteAll() {
    const result = await this.Question_quiz_bank.deleteMany({});
    return result.deletedCount;
  }
}
module.exports = Question_quiz_bankService;
