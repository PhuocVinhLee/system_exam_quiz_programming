...
const { get_all_periods,get_all_periods_peroid_teacher,get_one_periods } = storeToRefs(usePeriodStore());
const { action_all_periods,action_add_period,state_period,action_update_period, action_one_periods,reset_state_period} = usePeriodStore();

//const courses_local = reactive({});

const period_local = ref({
...
async function submit() {
    ...
    await action_update_period(route.params.period_id,period_local.value); // them courses vao csdl
...
onMounted(async () => {
await action_one_periods(route.params.period_id);
period_local.value = get_one_periods.value;
})
