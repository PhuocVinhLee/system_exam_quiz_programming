...
const { action_all_subjects,action_add_subject,state_subject,action_update_subject, action_one_subjects,reset_state_subject} = useSubjectStore();

//const courses_local = reactive({});

const subject_local = ref({
    subject_title: "Công nghệ web new",
...
async function submit() {
    ...
    await action_update_subject(route.params.subject_id,subject_local.value); // them courses vao csdl
...
onMounted(async () => {
await action_one_subjects(route.params.subject_id);
subject_local.value = get_one_subjects.value;
})
