...
<template>
    <div>
        <Form @submit="submit()" class="row " id="exam_add" :validation-schema="productFormSchema" v-slot="{ errors }">
            <!-- <div class="col-md-12">
                               <span v-html="question_quiz_bank.question_quiz_bank_describe"></span>
                            </div> -->
            <div class="col-md-12 mb-3">
                <label for="validationServer" class="form-label">Tên câu hỏi</label>
                <Field type="text" class="form-control"
                    :class="{ ' is-valid': !errors.question_quiz_bank_title && question_quiz_bank.question_quiz_bank_title }"
                    v-bind:class="{ 'is-invalid': errors.question_quiz_bank_title }"
                    name="question_quiz_bank_title"
                    v-model="question_quiz_bank.question_quiz_bank_title" />
                <ErrorMessage name="question_quiz_bank_title" class="error-feedback" />
                <span> {{ errors.question_quiz_bank_title }} </span>
            ...
            <!-- <div class="col-md-6 mb-3">
                ...
                <Field class="form-select "
                    :class="{ ' is-valid': !errors.question_quiz_bank_type && question_quiz_bank.question_quiz_bank_type }"
                    v-bind:class="{ 'is-invalid': errors.question_quiz_bank_type }" as="select"
                    name="question_quiz_bank_type" required aria-label="select example"
                    v-model="question_quiz_bank.question_quiz_bank_type">

                    <option value="private">Private</option>
                    <option value="public">Public</option>

                </Field>
                <ErrorMessage name="question_quiz_bank_type" class="error-feedback" />
            </div> -->
            <div class="col-md-6"></div>
            <!-- <div class="col-md-6 mb-3">
                <label for="validationServer" class="form-label">Quyền người dùng</label>
                <Field class="form-select "
                    :class="{ ' is-valid': !errors.question_quiz_bank_authorization && question_quiz_bank.question_quiz_bank_authorization }"
                    v-bind:class="{ 'is-invalid': errors.question_quiz_bank_authorization }" as="select"
                    name="question_quiz_bank_authorization" required aria-label="select example"
                    v-model="question_quiz_bank.question_quiz_bank_authorization">

                    <option value="use">Use</option>
                    <option value="write">Write</option>

                </Field>
                <ErrorMessage name="question_quiz_bank_authorization" class="error-feedback" />
            </div> -->
            ...
            <div class="mb-3">
                ...
                <button v-on:click.self="list_teacher = false" type="button" class="btn btn-primary" data-bs-toggle="modal"
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mt-3" @click.self="list_teacher = true">
                                    ...
                                    <Field type="text" class="form-control" v-on:keyup.self="search_teacher()"
                                        :class="{ ' is-valid': !errors.list_teacher_id && question_quiz_bank.list_teacher_id }"
...
//const { question_quiz_bank } = storeToRefs(useQuestion_quiz_bankStore);
const { one_question_quiz_bank, get_one_question_quiz_bank, list_question_quiz_banks, lenght, get_list_question_quiz_bank } = storeToRefs(useQuestion_quiz_bankStore());
const { action_one_question_quiz_bank, action_list_question_quiz_bank, action_add_question_quiz_bank, reset_state_question_quiz_bank } = useQuestion_quiz_bankStore();




const question_quiz_bank = reactive({
    question_quiz_bank_title: " Môn công nghệ web AA TN ",
...
async function submit() {
    ...
    await action_add_question_quiz_bank(question_quiz_bank);
    auto_close_toast();// tu tat thong bao

    router.push({
        name: "QuestionQuizBankAll", params:{id_subject: route.query.id}
    }
    );
